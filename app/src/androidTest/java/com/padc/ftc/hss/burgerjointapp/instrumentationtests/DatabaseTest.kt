package com.padc.ftc.hss.burgerjointapp.instrumentationtests
...
class DatabaseTest {
    ...
    // CRUD operation (need db, dao)
    private lateinit var burgerDao: BurgerDao
    private lateinit var burgerJointDatabase: BurgerJointDatabase

    @Before
    fun createDb() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        burgerJointDatabase = Room.inMemoryDatabaseBuilder(
            context, BurgerJointDatabase::class.java
        ).build()
        burgerDao = burgerJointDatabase.getBurgerDao()
    }

    @After
    fun closeDb() {
        burgerJointDatabase.close()
    }

    @Test
    fun insertIntoDatabaseTest() {
        val burgerToInsert = BurgerVO()
        burgerToInsert.burgerId = 111
        burgerToInsert.burgerName = "Chicken Burger"
        burgerToInsert.burgerDescription =
        ...
        burgerToInsert.burgerImageUrl =
            "https://vignette.wikia.nocookie.net/simpsons-restaurants/images/2/20/Spicy_Clucker.png/revision/latest?cb=20131125185837"

        //add to db
        burgerDao.insert(burgerToInsert)

        //check added data's id exist or not in db
        assert(burgerDao.findBurgerByIdOneShot(burgerToInsert.burgerId).burgerId == burgerToInsert.burgerId)
    }
